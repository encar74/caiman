...
@include('includes.errors')
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-2">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control input-sm" id="lote" placeholder="0" value="">
                ...
                <table class="table mb-0 thead-border-top-0 table-striped">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 50px; text-align:center; ">Lote</th>
    ...
    @foreach($lineasPedido as $index => $linea)
        @php
            ...
            // Buscar el producto correspondiente manualmente
            foreach($productos as $producto) {
                if ($producto['id'] == $linea->articulo_id) {
            ...
            <td>
                <div class="media align-items-center">
                    <div class="media-body">
                        ...
                        <input type="hidden" name="lineas_pedido[{{ $index }}][lote]" value="{{ $linea->lote }}">
...
$(document).ready(function() {
    ...
    var importeIVA = 0;
    var totalPedido = 0;
    var totalLinea = 0; // Mover la declaración de totalLinea aquí
...
function calcularTotales() {
    ...
    $('.list tr').each(function(index, fila) {
        if (index < numFilas - 1) {
            var totalLinea = parseFloat($(fila).find('td:eq(4)').text().replace('€', ''));
            var ivaProducto = parseFloat($(fila).find('td:eq(6)').text().replace('%', ''));
            var totalLineaConIVA = parseFloat($(fila).find('td:eq(7)').text().replace('€', ''));
    ...
    // Verificar si ya existen líneas de pedido en la tabla
    if (numFilas > 1) {
        // Obtener los valores de las líneas de pedido existentes y sumarlos a los totales
        $('.list tr').each(function(index, fila) {
            if (index < numFilas - 1) {
                var totalLinea = parseFloat($(fila).find('td:eq(4)').text().replace('€', ''));
                var ivaProducto = parseFloat($(fila).find('td:eq(5)').text().replace('%', ''));
                var totalLineaConIVA = parseFloat($(fila).find('td:eq(6)').text().replace('€', ''));
                baseImponible += totalLinea;
                importeIVA += totalLineaConIVA - totalLinea;
                totalPedido += totalLineaConIVA;
            }
...
calcularTotales();
    ...
    $('#btnAdd').click(function(event) {
        ...
        if (!isNaN(cantidad) && !isNaN(precio)) {
            ...
            // Sumar el nuevo total con IVA al total del pedido
            baseImponible += totalLineaSinIva;
            importeIVA += totalLineaConIva - totalLineaSinIva;
            totalPedido += totalLineaConIva;
            ...
            var newRow = `
                <tr>
                    ...
                    <td align='center'>${lote}
                        <input type="hidden" name="lineas_pedido[${index}][lote]" value="${lote}">
                    </td>
                    ...
                    <td align='center'>${ivaProducto}%</td>
...
$(document).on('click', '.btn-delete', function() {
    ...
    if (!lineaPedidoId) {
        console.log("Eliminando fila sin ID:", row);
        row.remove();
        calcularTotales();
    ...
    $.ajax({
        url: '{{ route("orders.deleteLinea", ":id") }}'.replace(':id', lineaPedidoId), // Usar lineaPedidoId como el ID de la línea de pedido
        type: 'POST',
        data: {
            _method: 'POST',
            _token: "{{ csrf_token() }}",
            id: lineaPedidoId // Pasar lineaPedidoId como el ID de la línea de pedido
        },
        success: function(data) {
            console.log(data); // Agrega esto para depurar la respuesta recibida desde el servidor
            // Verificar si la solicitud se completó correctamente
            if (data.success) {
                // Restar los valores de la fila eliminada de los totales globales
                var totalLineaEliminar = parseFloat(row.find('td:eq(4)').text().replace('€', ''));
                var ivaEliminar = parseFloat(row.find('td:eq(5)').text().replace('%', ''));
                var totalLineaConIvaEliminar = parseFloat(row.find('td:eq(6)').text().replace('€', ''));

                baseImponible -= totalLineaEliminar;
                importeIVA -= totalLineaConIvaEliminar - totalLineaEliminar;
                totalPedido -= totalLineaConIvaEliminar;

                // Registrar la fila que se eliminará en la consola
                ...
                // Manejar el caso en que ocurra un error al eliminar la línea de pedido
                alert('Error al eliminar la línea de pedido: ' + data.message);
            }
        },
        error: function(xhr, status, error) {
            // Manejar el caso en que ocurra un error con la solicitud AJAX
    ...
    // Agregar un evento de cambio al elemento #articulo
    $('#articulo').change(function() {
        // Obtener el nombre del artículo seleccionado
        var selectElement = $(this);
        var articuloId = parseInt(selectElement.find('option:selected').data('articulo-id'));
...
$(document).ready(function() {
    $('#cliente').change(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                // Actualizar el select de direcciones con las opciones obtenidas
                $('#destino').empty();
    ...
    $('#articulo').change(function() {
        ...
        if (articuloId) {
            $.ajax({
                ...
                success: function(data) {
                var articuloNombre = data.nombre_articulo;
                var articuloPrecio = parseFloat(data.precio); // Convertir precio a número
                // Verifica si el precio es un número válido antes de asignarlo
            ...
            // Resetea el campo de precio si no hay un artículo seleccionado
            $('#precio').val('');
        }
    });
});
